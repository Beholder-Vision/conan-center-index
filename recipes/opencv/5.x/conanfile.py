...
class OpenCVConan(ConanFile):
    ...
    def source(self):
        git = Git(self)
        git.clone(url=self.conan_data["sources"][self.version][0]["url"], target=".")
        git.checkout(self.conan_data["sources"][self.version][0]["ref"])

        git = Git(self, folder=self._contrib_folder)
        git.clone(url=self.conan_data["sources"][self.version][1]["url"], target=".")
        git.checkout(self.conan_data["sources"][self.version][1]["ref"])
